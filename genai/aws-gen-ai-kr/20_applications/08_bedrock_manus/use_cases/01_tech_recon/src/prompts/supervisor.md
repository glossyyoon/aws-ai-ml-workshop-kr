...
* **[CRITICAL LIMIT]**: Researcher can be called MAXIMUM 3 TIMES per workflow. Track the number of researcher calls carefully.
...
- **[CRITICAL RESEARCHER LIMIT]**: Researcher tool can be called MAXIMUM 3 TIMES total. Count researcher calls in the history/clues and stop calling researcher after reaching this limit.
...
- **[CRITICAL]** FIRST, count how many times researcher has been called by checking history/clues
...
- After a worker completes a task, evaluate if another worker is needed:
  - Switch to researcher ONLY if: (1) information gathering is required AND (2) researcher has been called LESS THAN 3 times
  - If researcher limit (3 calls) is reached, skip remaining research tasks or consolidate them
